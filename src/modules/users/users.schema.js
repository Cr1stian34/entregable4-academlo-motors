...
const registerSchema = z.object({
    ...
    password: z.string().min(8, {message: "password must be a last 8 characteres"}).max(16,{message: "maximo 16 caracteres"}),
    role: z.string()
})

const loginUserSchema = z.object({
    email: z.string().email({message: "invalid email"}),
    password: z.string().min(8, {message: "password mus be a last 8 characteres"}).max(16, {message: "maximo 16 caracteres"})
})
